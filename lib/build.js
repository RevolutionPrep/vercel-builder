...
async function build({ files, entrypoint, workPath, config = {}, meta = {} }) {
    ...
    // Node version
    const nodeVersion = await build_utils_1.getNodeVersion(rootDir, undefined, config, meta);
    ...
    const srcDir = nuxtConfigFile.srcDir ? path_1.default.relative(rootDir, nuxtConfigFile.srcDir) : '.';
    ...
    // Static files
    const staticFiles = await build_utils_1.glob('**', path_1.default.join(rootDir, srcDir, staticDir));
    // Client dist files
    ...
    const generatedDir = path_1.default.join(rootDir, 'dist');
    const generatedPagesFiles = config.generateStaticRoutes ? await utils_1.globAndPrefix('**/*.html', generatedDir, './') : {};
    // node_modules_prod
